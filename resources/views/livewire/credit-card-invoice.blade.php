<div class="bg-slate-800 p-6 rounded-lg shadow-md border border-slate-700">
    ...
    @else
    {{-- ABAS DOS CARTÕES E BOTÕES DE CONTROLE --}}
    <div class="flex flex-wrap items-center gap-2 mb-6 border-b border-slate-700 pb-4">
        @foreach($cards as $card)
        <button wire:click="selectCard({{ $card->id }})"
            @class([ 'px-5 py-2.5 rounded-lg text-sm font-bold transition-all duration-200 flex items-center gap-2 shadow-sm' , 'bg-slate-700 text-white border-slate-600 scale-105'=> $selectedCardId === $card->id,
        ...
        {{-- LINHA DIVISÓRIA E OS BOTÕES MÁGICOS --}}
        <div class="flex items-center gap-2 ml-2 pl-2 border-l border-slate-700">
            <button wire:click="$dispatch('trigger-create-card')" class="px-3 py-2.5 rounded-lg text-sm font-bold transition-all duration-200 bg-orange-500/10 text-orange-500 hover:bg-orange-500/20 border border-orange-500/30 flex items-center gap-1" title="Adicionar Novo Cartão">
                <span class="text-lg leading-none">+</span> <span class="hidden sm:inline">Novo</span>
            </button>

            @if($selectedCardId)
            <button wire:click="$dispatch('trigger-edit-card', { id: {{ $selectedCardId }} })" class="px-3 py-2 rounded-lg text-lg transition-all duration-200 text-slate-500 hover:text-blue-400 hover:bg-blue-400/10" title="Editar Configurações do Cartão">
                ✏️
            </button>
            @endif
        </div>
    ...
    <div class="bg-slate-900/50 rounded-xl border border-slate-700 p-1 md:p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div class="flex flex-col gap-3">
                <div @class([ 'rounded-xl p-6 text-white shadow-lg relative overflow-hidden flex flex-col justify-between h-48 transition-all' , 'bg-gradient-to-br from-purple-500 to-purple-800'=> $selectedCard->color === 'purple',
            ...
            <div class="bg-slate-900 p-6 rounded-xl border border-slate-700 shadow-sm flex flex-col justify-center space-y-6">
                ...
                <div>
                    ...
                    <div class="overflow-hidden h-3 text-xs flex rounded-full bg-slate-800">
                        <div {!! 'style="width: ' . min(100, $limitPercentage) . '%;"' !!} @class([ 'shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center transition-all duration-1000' , 'bg-green-500'=> $limitPercentage < 50, 'bg-yellow-500'=> $limitPercentage >= 50 && $limitPercentage < 85, 'bg-red-500'=> $limitPercentage >= 85,
                                    ])></div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="overflow-hidden h-3 text-xs flex rounded-full bg-slate-800 relative">
                        <div {!! 'style="width: ' . min(100, $metaPercentage) . '%;"' !!} @class([ 'shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center transition-all duration-1000' , 'bg-blue-500'=> $metaPercentage < 80, 'bg-orange-500'=> $metaPercentage >= 80 && $metaPercentage <= 100, 'bg-red-500 animate-pulse'=> $metaPercentage > 100,
                                    ])></div>
